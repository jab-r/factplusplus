...
Copyright (C) 2011-2013 Stanislav Kikot
...
public:		// interafce
		/// init c'tor
	explicit BuildELIOConcept ( const QRQuery* q ) : query(q) {}
		/// empty d'tor
	virtual ~BuildELIOConcept() = default;
...
protected:
	TConceptExpr* createConceptByVar ( const QRVariable* v ) override
	{
...
public:
		/// init c'tor
	explicit TermAssigner ( const QRQuery* query ) : BuildELIOConcept(query) { NewNominals.clear(); }
}; // TermAssigner
...
transformQueryPhase2 ( QRQuery* query )
{
	...
	return assigner.Assign (nullptr, var );
}
...
public:
		/// init c'tor
	explicit QueryApproximation ( const QRQuery* query ) : BuildELIOConcept(query) {}
}; // QueryApproximation
...
static void BuildAproximation ( const QRQuery* query )
{
	...
	for ( QRQuery::QRVarSet::const_iterator v = query->FreeVars.begin(), v_end = query->FreeVars.end(); v != v_end; ++v )
	{
		...
		approx[var] = And ( approx[var], app.Assign (nullptr, *v ) );
	}
...
public:		// interface
		/// init c'tor
	explicit TExpressionMarker ( TConceptExpr * nominal) : Nominal(nominal) {}

public:
	// concept expressions

	void visit ( const TDLConceptTop& expr ) override
	{
	...
	void Solve() {
		while(! isNominal(LeftPart)) {
			if ( const TDLConceptObjectExists * leftDiamond = dynamic_cast<const TDLConceptObjectExists *>(LeftPart)) {
				...
				if (dynamic_cast<const TDLConceptTop * > (arg2) == nullptr){
...
public:		// interface
		/// init c'tor
	explicit TQueryToConceptsTransformer ( QRQuery* query ) : Query(query) {}
